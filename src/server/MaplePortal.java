    ...
    public final void enterPortal(final MapleClient c) {
        if (getPosition().distanceSq(c.getPlayer().getPosition()) > 40000) {
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().showInfo("無效傳送", true, "到傳送點的距離:" + getPosition().distanceSq(c.getPlayer().getPosition()) + "限制最大距離:" + 40000 + "傳送點坐標(" + getPosition().x + "," + getPosition().y + ")角色坐標(" + c.getPlayer().getPosition().x + "," + c.getPlayer().getPosition().y + ")");
            }
            if (c.getPlayer().getMapId() != 4000010) {
