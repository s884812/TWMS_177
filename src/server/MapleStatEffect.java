...
//                        break;
    ...
    public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final Point pos, int newDuration) {
        ...
        if (expinc != 0) {
            ...
            applyto.getClient().getSession().write(EffectPacket.showForeignEffect(22));
    ...
    private void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        switch (sourceid) {
            ...
            case 32111005: { //body boost
                if (applyfrom.getStatForBuff(MapleBuffStat.BODY_BOOST) != null) {
            ...
            case 32001003: { //黑色繩索
                if (applyfrom.getTotalSkillLevel(32120013) > 0) {
                    SkillFactory.getSkill(32120013).getEffect(applyfrom.getTotalSkillLevel(32120013)).applyBuffEffect(applyfrom, applyto, primary, newDuration);
                    return;
                }
            }
            case 32110007: //超級體-黑色繩索
            case 32120013: { //進階黑色繩索
                if (applyfrom.getStatForBuff(MapleBuffStat.DARK_AURA) != null && sourceid != 32110007) {
                    ...
                    localDuration = (sourceid == 32110007 ? (60 + 7 * level) : 2100000000);
                    statt.put(MapleBuffStat.DARK_AURA, sourceid == 32110007 ? (info.get(MapleStatInfo.x) * 2) : info.get(MapleStatInfo.x));
                    if (sourceid == 32110007) {
                        if (applyfrom.getTotalSkillLevel(32120013) > 0) {
                            sourceid = 32120013;
                        } else {
                            sourceid = 32001003;
            ...
            case 32111012: { //藍色繩索
                if (applyfrom.getTotalSkillLevel(32120015) > 0) {
                    SkillFactory.getSkill(32120015).getEffect(applyfrom.getTotalSkillLevel(32120015)).applyBuffEffect(applyfrom, applyto, primary, newDuration);
                    return;
                }
            }
            case 32110008: //超級體-藍色繩索
            case 32120015: { //進階藍色繩索
                if (applyfrom.getStatForBuff(MapleBuffStat.BLUE_AURA) != null && sourceid != 32110008) {
                    applyto.cancelEffectFromBuffStat(MapleBuffStat.BLUE_AURA);
                    return;
                } else {
                    final EnumMap<MapleBuffStat, Integer> statt = new EnumMap<>(MapleBuffStat.class);
                    localDuration = (sourceid == 32110008 ? (60 + 7 * level) : 2100000000);
                    statt.put(MapleBuffStat.BLUE_AURA, (int) level);
                    statt.put(MapleBuffStat.HP_BOOST, (int) sourceid == 32110008 ? (info.get(MapleStatInfo.indieMhp) * 2) : info.get(MapleStatInfo.indieMhp));
                    statt.put(MapleBuffStat.MDEF_BOOST, (int) sourceid == 32110008 ? (info.get(MapleStatInfo.indieMdd) * 2) : info.get(MapleStatInfo.indieMdd));
                    statt.put(MapleBuffStat.WDEF_BOOST, (int) sourceid == 32110008 ? (info.get(MapleStatInfo.indiePdd) * 2) : info.get(MapleStatInfo.indiePdd));
            ...
            case 32101003: { // 黃色繩索
                if (applyfrom.getTotalSkillLevel(32120014) > 0) {
                    SkillFactory.getSkill(32120014).getEffect(applyfrom.getTotalSkillLevel(32120014)).applyBuffEffect(applyfrom, applyto, primary, newDuration);
                    return;
                }
            }
            case 32110009: //超級體-黃色繩索
            case 32120014: { //進階黃色繩索
                if (applyfrom.getStatForBuff(MapleBuffStat.YELLOW_AURA) != null && sourceid != 32110009) {
                    applyto.cancelEffectFromBuffStat(MapleBuffStat.YELLOW_AURA);
                    return;
                } else {
                    final EnumMap<MapleBuffStat, Integer> statt = new EnumMap<>(MapleBuffStat.class);
                    localDuration = (sourceid == 32110009 ? (60 + 7 * level) : 2100000000);
                    statt.put(MapleBuffStat.YELLOW_AURA, (int) sourceid == 32110009 ? (info.get(MapleStatInfo.x) * 2) : info.get(MapleStatInfo.x));
                    if (sourceid == 32110009) {
                        if (applyfrom.getTotalSkillLevel(32120014) > 0) {
                            sourceid = 32120014;
                        } else {
                            sourceid = 32101003;
            ...
            case 40011186: { // 劍豪初心者拔刀術
                ...
                statt.put(MapleBuffStat.SPEED, 10);
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, localDuration, statt, this));
                applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), statt, this), false);
                normal = false;
                break;
            ...
            case 41001001: { // 拔刀術                            
                if (applyfrom.getStatForBuff(MapleBuffStat.HAYATO) != null) {
                    applyto.cancelEffectFromBuffStat(MapleBuffStat.HAYATO);
                    return;
                }
                final EnumMap<MapleBuffStat, Integer> statt = new EnumMap<>(MapleBuffStat.class);
                localDuration = 2100000000;
                statt.put(MapleBuffStat.CRITICAL_RATE, info.get(MapleStatInfo.y));
                statt.put(MapleBuffStat.HAYATO, info.get(MapleStatInfo.prop));
                statt.put(MapleBuffStat.BATTOUJUTSU_STANCE, -1);
                if (applyfrom.getTotalSkillLevel(41110008) > 0) {
                    statt.put(MapleBuffStat.BATTOUJUTSU_SOUL, 0);
                }
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, localDuration, statt, this));
                applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), statt, this), false);
                normal = false;
            ...
            case 41121015: { // 制敵之先
                if (applyfrom.getStatForBuff(MapleBuffStat.COUNTERATTACK) != null) {
                    applyto.cancelEffectFromBuffStat(MapleBuffStat.COUNTERATTACK);
                    return;
                }
                final EnumMap<MapleBuffStat, Integer> statt = new EnumMap<>(MapleBuffStat.class);
                localDuration = 2100000000;
                statt.put(MapleBuffStat.COUNTERATTACK, 0);
                applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, localDuration, statt, this));
                applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), statt, this), false);
                normal = false;
                break;
            ...
            default:
                if (isPirateMorph()) {
                    ...
                    if (mountid != 0 && mountid2 != 0) {
                        ...
                        applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, 0, stat2, this));
