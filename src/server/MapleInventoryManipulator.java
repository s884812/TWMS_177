...
public class MapleInventoryManipulator {
    ...
    public static boolean addById(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, int periodMod, String gmLog) {
        return addId(c, itemId, quantity, owner, pet, period, periodMod, gmLog) >= 0;
    }

    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, int periodMod, String gmLog) {
        ...
        if (period > 0) {
            switch (periodMod) {
                case 3://MapleInventoryManipulator.DAY
                    period *= 24;
                case 2://MapleInventoryManipulator.HOUR
                    period *= 60;
                case 1://MapleInventoryManipulator.MINUTE
                    period *= 60;
                case 0://MapleInventoryManipulator.SECOND
                    period *= 1000;
                    break;
            }
        }
    ...
    public static Item checkEnhanced(final Item before, final MapleCharacter chr) {
        if (before instanceof Equip) {
            ...
            if (eq.getState() == 0 && (eq.getUpgradeSlots() >= 1 || eq.getLevel() >= 1) && GameConstants.canScroll(eq.getItemId()) && Randomizer.nextInt(100) >= 80) { //20%概率潛能掉寶
                eq.resetPotential();
            }
        }
        return before;
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (dst > -1200 && dst < -999 && !GameConstants.isEvanDragonItem(source.getItemId()) && !GameConstants.isMechanicItem(source.getItemId())) {
            ...
            if (chr.isShowErr()) {
                chr.dropMessage(5, new StringBuilder().append("穿戴裝備 - 2 ").append(source.getItemId()).append(" dst: ").append(dst).append(" 檢測1: ").append(dst <= -1200).append(" 檢測2: ").append((dst >= -5003) && (dst < -99)).append(" 檢測3: ").append(!stats.containsKey("cash")).toString());
            }
            c.getSession().write(CWvsContext.enableActions());
            return;
        } else if ((dst <= -1300) && (dst > -1306) && (!MapleJob.is天使破壞者(chr.getJob()))) {
            if (chr.isShowErr()) {
                chr.dropMessage(5, new StringBuilder().append("穿戴裝備 - 3 ").append(source.getItemId()).append(" dst: ").append(dst).append(" 檢測1: ").append((dst <= -1300) && (dst > -1306)).append(" 檢測2: ").append(!MapleJob.is天使破壞者(chr.getJob())).toString());
            }
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        if ((dst == -35) && (!GameConstants.isEnergy(source.getItemId()))) {
        ...
        if (dst == -36) { //pendant
            ...
            if (stat == null || stat.getCustomData() == null || (Long.parseLong(stat.getCustomData()) != 0 && Long.parseLong(stat.getCustomData()) < System.currentTimeMillis())) {
                c.getSession().write(CWvsContext.enableActions());
                return;
            }
        }
        ...
        switch (dst) {
            ...
            case -10: { // Shield
                ...
                if (GameConstants.isKatara(source.getItemId())) {
                    if ((chr.getJob() != 900 && !MapleJob.is影武者(chr.getJob())) || weapon == null || !GameConstants.isDagger(weapon.getItemId())) {
                        c.getSession().write(InventoryPacket.getInventoryFull());
                        c.getSession().write(InventoryPacket.getShowInventoryFull());
                        return;
                    }
        ...
        } else if (dst > -1300 && dst <= -1200 && chr.getAndroid() != null) {
            chr.updateAndroid(true);
        }
        ...
        source.setPosition(takeOff == 0 ? dst : takeOff);
        ...
        if (source.getState() >= 17) {
            ...
            int[] potentials = {source.getPotential1(), source.getPotential2(), source.getPotential3(), source.getBonusPotential1(), source.getBonusPotential2(), source.getBonusPotential3()};
        ...
        c.getSession().write(InventoryPacket.moveInventoryItem(MapleInventoryType.EQUIP, src, takeOff == 0 ? dst : takeOff, (byte) 2, false, false));
        if (takeOff != 0) {
            c.getSession().write(InventoryPacket.moveInventoryItem(MapleInventoryType.EQUIP, dst, src, (byte) 2, false, false));
        }
        chr.equipChanged();
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (GameConstants.isWeapon(source.getItemId())) {
            ...
            c.getPlayer().removeAndroid();
            c.getSession().write(CField.removeAndroidHeart());
        } else if (src > -1300 && src <= -1200 && c.getPlayer().getAndroid() != null) {
            c.getPlayer().updateAndroid(true);
        } else if (source.getItemId() == 1122017) {
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        if (quantity < source.getQuantity() && !GameConstants.isRechargable(source.getItemId())) {
            ...
            if (GameConstants.isPet(source.getItemId()) || ItemFlag.UNTRADABLE.check(flag) || ii.isDropRestricted(target.getItemId()) || ii.isAccountShared(target.getItemId())) {
            ...
            if (GameConstants.isPet(source.getItemId()) || ItemFlag.UNTRADABLE.check(flag) || ii.isDropRestricted(source.getItemId()) || ii.isAccountShared(source.getItemId())) {
                if (ItemFlag.KARMA_EQ.check(flag)) {
