...
public class MapleQuest implements Serializable {
    ...
    public void complete(MapleCharacter c, int npc, Integer selection) {
        if (c.getMap() != null && (autoPreComplete || checkNPCOnMap(c, npc)) && canComplete(c, npc)) {
            ...
            c.getClient().getSession().write(EffectPacket.showForeignEffect(0xE)); // 任務完成
            c.getMap().broadcastMessage(c, EffectPacket.showForeignEffect(c.getId(), 0xE), false); // 任務完成
        }
    }
    ...
    private boolean checkNPCOnMap(MapleCharacter player, int npcid) {
        //mir = 1013000
        return (MapleJob.is龍魔導士(player.getJob()) && npcid == 1013000) || npcid == 9000040 || npcid == 9000066 || (player.getMap() != null && player.getMap().containsNPC(npcid));
    }
