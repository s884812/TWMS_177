...
public class CharLoginHandler {
    ...
    public static final void login(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (loginok == 0 && (ipBan || macBan) && !c.isGM()) {
            ...
            //選擇性别並設置第二組密碼
            c.updateLoginState(4, c.getSessionIPAddress());
            ...
            if (ServerConfig.isAutoRegister()) {
                ...
                if (AutoRegister.success) {
                    c.getSession().write(CWvsContext.broadcastMsg(1, "註冊賬號成功。\r\n請重新輸入賬號密碼進入遊戲。"));
                } else {
                    c.getSession().write(CWvsContext.broadcastMsg(1, "註冊賬號失敗。"));
                }
            } else {
                c.getSession().write(CWvsContext.broadcastMsg(1, "賬號註冊失敗，未開啟自動註冊功能，請到網站註冊賬號。"));
            }
            loginok = 1;
        }
    ...
    public static void ServerListRequest(final MapleClient c, boolean Return) {
        if (Return) {
            return;
        }
    ...
    public static void CreateChar(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        int[][] skills = new int[][]{
            ...
            {20010022/*龍飛行*/, 20010194/*繼承的意志*/},//龍魔導士Evan
            {20020109/*精靈的回復*/, 20020111/*時髦的移動*/, 20020112/*王的資格*/}, //精靈遊俠Mercedes
            {},//惡魔Demon
            {},//幻影俠盜Phantom
            {},//影武者Dualblade
            {},//米哈逸Mihile
            ...
            {},//凱薩Kaiser
            {60011216/*繼承人*/, 60010217/*真正的繼承人*/, 60011218/*魔法起重機*/, 60011220/*白日夢*/, 60011221/*配飾*/, 60011222/*魔法變身*/},//天使破壞者AngelicBuster
            {},//重炮指揮官Cannoneer
    ...
    public static void SetGender(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (c.getAccountName().equals(name) && gender >= 0 && gender <= 1 && secondPassword.length() >= 5) {
