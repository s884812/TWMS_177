    ...
    public static void handlePacket(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (code) {
            ...
            case 1: { //星力強化
                ...
                if (item.getUpgradeSlots() > 0 || item.getEnhance() >= GameConstants.getEnhanceTimes(item.getItemId()) || ii.isCash(item.getItemId()) || maplePoint ? c.getPlayer().getCSPoints(2) < meso : c.getPlayer().getMeso() < meso) {
                    c.getSession().write(CWvsContext.Enchant.getStarForceResult(c.getPlayer(), item, item, StarForceResult.失敗.getValue()));
                    if (c.getPlayer().isShowErr()) {
                        c.getPlayer().showInfo("星力強化", true, "星力強化檢測 裝備是否有剩餘升級次數：" + (item.getUpgradeSlots() >= 1) + " 裝備星級是否大於可升級的星數：" + (item.getEnhance() >= GameConstants.getEnhanceTimes(item.getItemId())) + " 裝備是否是為點裝：" + ii.isCash(item.getItemId()) + (maplePoint ? " 楓點" : " 楓幣") + "不足：" + (maplePoint ? c.getPlayer().getCSPoints(2) < meso : c.getPlayer().getMeso() < meso));
                    }
                    return;
                }
            ...
            case 50: { // FIXME 咒文的痕跡 放置道具
                ...
                slea.skip(2);
                Equip item = (Equip) c.getPlayer().getInventory(position < 0 ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP).getItem(position);
                c.getSession().write(CWvsContext.Enchant.getEnchantList(c, item, getEchantScrolls(item), ServerConstants.feverTime));
                break;
            }
            case 52: { //星力強化 放置道具
                short position = slea.readShort();
