...
//                c.getPlayer().getMap().broadcastMessage(MainPacketCreator.MegidoFlameRe(c.getPlayer().getId(), unk, slea.readInt()));
    ...
    public static void TakeDamage(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
        ...
        if ((chr.getStat().dodgeChance > 0) && (Randomizer.nextInt(100) < chr.getStat().dodgeChance)) {
            c.getSession().write(CField.EffectPacket.showForeignEffect(22));
            return;
        }
    ...
    public static void KaiserQuickKey(LittleEndianAccessor slea, MapleCharacter chr) {
        if (MapleJob.is凱撒(chr.getJob()) && (chr != null)) {
            String ss = chr.getQuestNAdd(MapleQuest.getInstance(52554)).getCustomData();
            String cmd0 = !ss.contains("cmd0") ? "" : ss.substring(ss.indexOf("cmd0"), ss.indexOf("cmd0") + 13);
            String cmd1 = !ss.contains("cmd1") ? "" : ss.substring(ss.indexOf("cmd1"), ss.indexOf("cmd1") + 13);
            String cmd2 = !ss.contains("cmd2") ? "" : ss.substring(ss.indexOf("cmd2"), ss.indexOf("cmd2") + 13);
            ...
            for (int i = 0; i < count; i++) {
                ...
                switch (tmp1) {
                    case 0:
                        skill1 = slea.readInt();
                        cmd0 = "cmd" + tmp1 + "=" + skill1;
                        break;
                    case 1:
                        skill2 = slea.readInt();
                        cmd1 = "cmd" + tmp1 + "=" + skill2;
                        break;
                    case 2:
                        skill3 = slea.readInt();
                        cmd2 = "cmd" + tmp1 + "=" + skill3;
                        break;
                }
            }
            StringBuilder ret = new StringBuilder();
            ret.append(cmd0).append(";").append(cmd1).append(";").append(cmd2);            
            System.out.println("KaiserQuickKey : " + ret);
            chr.getQuestNAdd(MapleQuest.getInstance(52554)).setCustomData(ret.toString());
            chr.getClient().getSession().write(CField.sendKaiserQuickKey(skill1, skill2, skill3));
        }
    }
...
//        System.err.println(skillLevel);
        ...
        switch (skillid) {
            ...
            case 110001501:
                slea.skip(3);
                stat.clear();
                stat.put(MapleBuffStat.ANIMAL_SELECT, 1);
                chr.setBuffedValue(MapleBuffStat.ANIMAL_SELECT, 1);
                c.getSession().write(CWvsContext.BuffPacket.giveBuff(skillid, 2100000000, stat, null));
                c.getSession().write(CWvsContext.enableActions());
                break;
            case 110001502:
                slea.skip(3);
                stat.clear();
                stat.put(MapleBuffStat.ANIMAL_SELECT, 2);
                chr.setBuffedValue(MapleBuffStat.ANIMAL_SELECT, 2);
                c.getSession().write(CWvsContext.BuffPacket.giveBuff(skillid, 2100000000, stat, null));
                c.getSession().write(CWvsContext.enableActions());
                break;
            case 110001503:
                slea.skip(3);
                stat.clear();
                stat.put(MapleBuffStat.ANIMAL_SELECT, 3);
                chr.setBuffedValue(MapleBuffStat.ANIMAL_SELECT, 3);
                c.getSession().write(CWvsContext.BuffPacket.giveBuff(skillid, 2100000000, stat, null));
                c.getSession().write(CWvsContext.enableActions());
                break;
            case 110001504:
                slea.skip(3);
                stat.clear();
                stat.put(MapleBuffStat.ANIMAL_SELECT, 4);
                chr.setBuffedValue(MapleBuffStat.ANIMAL_SELECT, 4);
                c.getSession().write(CWvsContext.BuffPacket.giveBuff(skillid, 2100000000, stat, null));
                c.getSession().write(CWvsContext.enableActions());
                break;
            default:
        ...
        switch (header) {
            ...
            case PASSIVE_ATTACK: // 被動攻擊, BUFF觸發的, 需要有BUFF才有效果
                if (chr.getBuffedValue(MapleBuffStat.ENERGY_CHARGE) == null && //能量获得
                        ...
                        chr.getBuffedValue(MapleBuffStat.DARK_AURA) == null && //黑暗灵气
