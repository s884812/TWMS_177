...
public class InventoryHandler {
    ...
    public static void UseGoldenHammer(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (GameConstants.canHammer(equip.getItemId()) && ii.getSlots(equip.getItemId()) > 0 && equip.getViciousHammer() < 1) {
    ...
    public final static void UseSoulEnchanter(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {//魂卷
        ...
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        if (!GameConstants.isSoulEnchanter(enchanter.getItemId()) || nEquip.getSoulEnchanter() != 0 || !GameConstants.isWeapon(nEquip.getItemId())) {
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().showMessage(11, "砸卷錯誤:使用的捲軸不是靈魂捲軸-" + !GameConstants.isSoulEnchanter(enchanter.getItemId()) + " 裝備已經上了魂卷-" + (nEquip.getSoulEnchanter() != 0) + " 使用捲軸的裝備不是武器-" + (!GameConstants.isWeapon(nEquip.getItemId())));
            }
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
    ...
    public final static void UseSoulScroll(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {//魂珠
        ...
        int skillid = GameConstants.getSoulSkill(soulid);
        Equip nEquip = (Equip) equip;
        if (!GameConstants.isSoulScroll(soulid) || skillid == 0 || nEquip.getSoulEnchanter() == 0 || !GameConstants.isWeapon(nEquip.getItemId())) {
            if (c.getPlayer().isShowErr()) {
                c.getPlayer().showMessage(11, "砸卷錯誤:使用的捲軸不是靈魂寶珠-" + !GameConstants.isSoulScroll(soulid) + " 寶珠未添加技能-" + (skillid == 0) + " 未上魂卷-" + (nEquip.getSoulEnchanter() == 0) + " 使用捲軸的裝備不是武器-" + (!GameConstants.isWeapon(nEquip.getItemId())));
            }
            c.getSession().write(CWvsContext.enableActions());
            return;
        }
        ...
        nEquip.setSoulPotential(GameConstants.getSoulPotential(soulid, nEquip));
        nEquip.setSoulSkill(GameConstants.getSoulSkill(soulid));
    ...
    public static boolean magnifyEquip(final MapleClient c, Item magnify, Item toReveal, byte eqSlot, boolean useMagnify, boolean show) {
        ...
        if ((eqq.getState() < 17 && eqq.getState() > 0 || eqq.getBonusState() < 17 && eqq.getBonusState() > 0) && (((price != -1 && c.getPlayer().getMeso() >= price) || price == -1) || magnify.getItemId() == 2460005 || magnify.getItemId() == 2460004 || magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && reqLevel <= 12) || (magnify.getItemId() == 2460001 && reqLevel <= 7) || (magnify.getItemId() == 2460000 && reqLevel <= 3))) {
            final List<List<StructItemOption>> pots = new LinkedList<>(ii.getAllPotentialInfo().values());
            int new_state;
            boolean isBonus = !(eqq.getState() < 17 && eqq.getState() > 0);
            boolean excludeUseless = false;
            boolean maxPotLevel = false;
            boolean twins = false;
            int lines;
            int lockedLine = 0;
            int locked = 0;
            if (!isBonus) {
                new_state = Math.abs(eqq.getPotential1());
                lines = eqq.getPotential2() != 0 ? 3 : 2;
            } else {
                new_state = Math.abs(eqq.getBonusPotential1());
                lines = eqq.getBonusPotential2() != 0 ? 3 : 2;
            }

            if (new_state > 100000000) {
                twins = true;
                new_state %= 100000000;
            }

            if (new_state > 10000000) {
                excludeUseless = true;
                new_state %= 10000000;
            }

            if (new_state > 1000000) {
                maxPotLevel = true;
                new_state %= 1000000;
            }

            if (new_state > 100000) {
                lockedLine = (new_state - (new_state % 100000)) / 100000;
                new_state %= 100000;
                locked = new_state;
            }
            ...
            if (lockedLine != 0) {
                if (!isBonus) {
                    new_state = Math.abs(eqq.getPotential3());
                    eqq.setPotential3(0);
                } else {
                    new_state = Math.abs(eqq.getBonusPotential3());
                    eqq.setBonusPotential3(0);
                }
            }
            ...
            //鑒定潛能
            boolean setPotentialOK = false;
            StructItemOption[] maxPots = new StructItemOption[]{null, null, null};
            while (!setPotentialOK) {
                ...
                for (int i = 0; i < lines; i++) { // minimum 2 lines, max 5
                    ...
                    while (!rewarded) {
                        ...
                        if (pot != null && pot.reqLevel / 10 <= reqLevel && GameConstants.optionTypeFits(pot.optionType, eqq.getItemId()) && GameConstants.potentialIDFits(pot.opID, new_state, i)) { //optionType
                            ...
                            if (GameConstants.isAllowedPotentialStat(eqq, pot.opID, isBonus) && (!excludeUseless || (excludeUseless && !GameConstants.isUselessPotential(pot)))) {
                                if (!isBonus) {
                                    if (i == 0) {
                ...
                setPotentialOK = (!isBonus && eqq.getState() == new_state) || (isBonus && eqq.getBonusState() == new_state);
            }

            //調整單條最高潛能
            int num = 0;
            for (StructItemOption opt : maxPots) {
                if (opt == null) {
                    num++;
                    continue;
                }
                String[] potential = opt.getItemOption();
                ArrayList<StructItemOption> pps = new ArrayList();
                for (Object pList : pots) {
                    List pL = (List) pList;
                    StructItemOption pot = (StructItemOption) pL.get(reqLevel);
                    int i = 0;
                    ...
                    if (potential.length == i && pot.optionType == opt.optionType) {
                        pps.add(pot);
                    }
                }
                StructItemOption maxPot = opt;
                for (StructItemOption pot : pps) {
                    String[] itemOption = pot.getItemOption();
                    int n = 0;
                    for (int i = 0; i < itemOption.length; i++) {
                        if (pot.get(itemOption[i]) >= maxPot.get(itemOption[i]) && pot.reqLevel / 10 <= reqLevel && GameConstants.optionTypeFits(pot.optionType, eqq.getItemId()) && GameConstants.potentialIDFits(pot.opID, new_state, i) && GameConstants.isAllowedPotentialStat(eqq, pot.opID, isBonus)) {
                            n++;
                            if (itemOption.length == n) {
                                maxPot = pot;
                            }
                        }
                    }
                ...
                if (maxPot != opt) {
                    if (!isBonus) {
                        if (num == 0) {
                            eqq.setPotential1(maxPot.opID);
                            if (twins) {
                                eqq.setPotential2(maxPot.opID);
                            }
                        } else if (num == 1 && !twins) {
                            eqq.setPotential2(maxPot.opID);
                        } else if (num == 2) {
                            eqq.setPotential3(maxPot.opID);
                        }
                    } else {
                        if (num == 0) {
                            eqq.setBonusPotential1(maxPot.opID);
                            if (twins) {
                                eqq.setBonusPotential2(maxPot.opID);
                            }
                        } else if (num == 1 && !twins) {
                            eqq.setBonusPotential2(maxPot.opID);
                        } else if (num == 2) {
                            eqq.setBonusPotential3(maxPot.opID);
                        }
                    }
                }
                num++;
            ...
            if (!isBonus) {
                switch (lockedLine) {
                    case 1:
                        eqq.setPotential1(locked);
                        break;
                    case 2:
                        eqq.setPotential2(locked);
                        break;
                    case 3:
                        eqq.setPotential3(locked);
                        break;
                }
            } else {
                switch (lockedLine) {
                    case 1:
                        eqq.setBonusPotential1(locked);
                        break;
                    case 2:
                        eqq.setBonusPotential2(locked);
                        break;
                    case 3:
                        eqq.setBonusPotential3(locked);
                        break;
                }
            }
            ...
            //放大鏡處理
            if (useMagnify) {
                if (!isBonus && eqq.getBonusState() < 17 && eqq.getBonusState() > 0) {
    ...
    public static boolean UseUpgradeScroll(final short slot, final short dst, final short ws, final MapleClient c, final MapleCharacter chr, final int vegas, final boolean legendarySpirit) {
        ...
        final byte oldBonusState = toScroll.getBonusState();
        ...
        if (GameConstants.isForGM(scroll.getItemId()) && !chr.isGM()) {
        ...
        if (GameConstants.isAzwanScroll(scroll.getItemId())) {//阿斯旺捲軸
        ...
        if (!GameConstants.isSpecialScroll(scroll.getItemId()) && !GameConstants.isCleanSlate(scroll.getItemId()) && !GameConstants.isEquipScroll(scroll.getItemId()) && !GameConstants.isPotentialScroll(scroll.getItemId()) && !GameConstants.isBonusPotentialScroll(scroll.getItemId())) {
        ...
        } else if (GameConstants.isEquipScroll(scroll.getItemId())) {//強化捲軸
            if (toScroll.getUpgradeSlots() >= 1 || toScroll.getEnhance() >= GameConstants.getEnhanceTimes(toScroll.getItemId()) || vegas > 0 || ii.isCash(toScroll.getItemId())) {
                if (chr.isShowErr()) {
                    chr.showInfo("砸卷錯誤", true, "強化捲軸檢測 裝備是否有升級次數：" + (toScroll.getUpgradeSlots() >= 1) + " 裝備星級是否大於可升級的星數：" + (toScroll.getEnhance() >= GameConstants.getEnhanceTimes(toScroll.getItemId())) + "vegas：" + (vegas > 0) + " 裝備是否是為點裝：" + ii.isCash(toScroll.getItemId()));
                }
                c.getSession().write(InventoryPacket.getInventoryFull());
                c.getSession().write(CWvsContext.enableActions());
                c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
        ...
        } else if (GameConstants.isPotentialScroll(scroll.getItemId())) {//潛能捲軸
            ...
            if ((!isEpic && !isUnique && !isLegend && toScroll.getState() >= 1) || (isEpic && toScroll.getState() >= 18) || (isUnique && toScroll.getState() >= 19) || (isLegend && toScroll.getState() >= 20) || (toScroll.getLevel() == 0 && toScroll.getUpgradeSlots() == 0 && !GameConstants.isShield(toScroll.getItemId()) && !GameConstants.isSpecialShield(toScroll.getItemId()) && !GameConstants.isEnergy(toScroll.getItemId()) && !GameConstants.isSpecialPotentialItem(toScroll.getItemId())/* && !isEpic && !isUnique*/) || vegas > 0 || ii.isCash(toScroll.getItemId()) || GameConstants.isSpecialCantPotentialItem(toScroll.getItemId())) {
                if (chr.isShowErr()) {
                    chr.showInfo("砸卷錯誤", true, "isPotentialScroll" + (toScroll.getState() >= 1) + " " + (toScroll.getLevel() == 0 && toScroll.getUpgradeSlots() == 0 && !GameConstants.isShield(toScroll.getItemId()) && !GameConstants.isSpecialShield(toScroll.getItemId()) && !GameConstants.isEnergy(toScroll.getItemId()) && !GameConstants.isSpecialPotentialItem(toScroll.getItemId())) + "vegas" + (vegas > 0) + "裝備是否是為點裝" + ii.isCash(toScroll.getItemId()) + "特殊潛能道具" + GameConstants.isSpecialCantPotentialItem(toScroll.getItemId()));
                }
                c.getSession().write(InventoryPacket.getInventoryFull());
                c.getSession().write(CWvsContext.enableActions());
                c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
        ...
        } else if (GameConstants.isBonusPotentialScroll(scroll.getItemId())) {//附加潛能捲軸
            if (toScroll.getBonusState() >= 1 || toScroll.getState() == 0 || (toScroll.getLevel() == 0 && toScroll.getUpgradeSlots() == 0 && !GameConstants.isShield(toScroll.getItemId()) && !GameConstants.isSpecialShield(toScroll.getItemId()) && !GameConstants.isEnergy(toScroll.getItemId()) && !GameConstants.isSpecialPotentialItem(toScroll.getItemId())) || vegas > 0 || ii.isCash(toScroll.getItemId()) || GameConstants.isSpecialCantPotentialItem(toScroll.getItemId())) {
                if (chr.isShowErr()) {
                    chr.showInfo("砸卷錯誤", true, "isBonusPotentialScroll " + (toScroll.getBonusState() >= 1) + " " + (toScroll.getLevel() == 0 && toScroll.getUpgradeSlots() == 0 && !GameConstants.isShield(toScroll.getItemId()) && !GameConstants.isSpecialShield(toScroll.getItemId()) && !GameConstants.isEnergy(toScroll.getItemId()) && !GameConstants.isSpecialPotentialItem(toScroll.getItemId())) + "vegas" + (vegas > 0) + "裝備是否是為點裝" + ii.isCash(toScroll.getItemId()) + "特殊潛能道具" + GameConstants.isSpecialCantPotentialItem(toScroll.getItemId()));
                }
                c.getSession().write(InventoryPacket.getInventoryFull());
                c.getSession().write(CWvsContext.enableActions());
                c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
        ...
        } else if (GameConstants.isSpecialScroll(scroll.getItemId())) {//特殊捲軸
            boolean re = false;
            if (ii.isCash(toScroll.getItemId())) {
                ...
                re = true;
            } else if (GameConstants.isLuckydayScroll(scroll.getItemId()) && ItemFlag.LUCKY_DAY.check(toScroll.getFlag())) {//幸運日
                if (chr.isShowErr()) {
                    chr.showInfo("砸卷錯誤", true, "已有效果" + ItemFlag.LUCKY_DAY.check(toScroll.getFlag()));
                }
                re = true;
            } else if (GameConstants.isSafeScroll(scroll.getItemId()) && ItemFlag.SLOTS_PROTECT.check(toScroll.getFlag())) {//安全捲軸
                if (chr.isShowErr()) {
                    chr.showInfo("砸卷錯誤", true, "已有效果" + ItemFlag.SLOTS_PROTECT.check(toScroll.getFlag()));
                }
                re = true;
            } else if (GameConstants.isScrollSafeScroll(scroll.getItemId()) && ItemFlag.SCROLL_PROTECT.check(toScroll.getFlag())) {//捲軸保護卡
                if (chr.isShowErr()) {
                    chr.showInfo("砸卷錯誤", true, "已有效果" + ItemFlag.SCROLL_PROTECT.check(toScroll.getFlag()));
                }
                re = true;
            }
            if (GameConstants.isProtectionScroll(scroll.getItemId())) {//保護捲軸
            ...
            switch (scroll.getItemId()) {
                ...
                case 5068200://寵物卷軸保護卡
                    if (GameConstants.isPetEquip(toScroll.getItemId())) {
                        break;
                    } else {
                        if (chr.isShowErr()) {
        ...
        } else if (GameConstants.isCleanSlate(scroll.getItemId())) {//白衣捲軸
        ...
        if (!GameConstants.canScroll(toScroll.getItemId()) && !GameConstants.isChaosScroll(toScroll.getItemId())) {
            if (chr.isShowErr()) {
                chr.showInfo("砸卷錯誤", true, "捲軸不能對裝備進行砸卷 " + !GameConstants.canScroll(toScroll.getItemId()) + " 不是混沌捲軸 " + !GameConstants.isChaosScroll(toScroll.getItemId()));
            }
            c.getSession().write(InventoryPacket.getInventoryFull());
            c.getSession().write(CWvsContext.enableActions());
            c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
        ...
        if ((GameConstants.isCleanSlate(scroll.getItemId()) || GameConstants.isTablet(scroll.getItemId()) || GameConstants.isGeneralScroll(scroll.getItemId()) || GameConstants.isChaosScroll(scroll.getItemId())) && (vegas > 0 || ii.isCash(toScroll.getItemId()))) {
            if (chr.isShowErr()) {
                chr.showInfo("砸卷錯誤", true, "捲軸是白醫捲軸 " + GameConstants.isCleanSlate(scroll.getItemId()) + " isTablet " + GameConstants.isTablet(scroll.getItemId()) + " 是普通捲軸 " + GameConstants.isGeneralScroll(scroll.getItemId()) + " 是混沌捲軸 " + GameConstants.isChaosScroll(scroll.getItemId()) + "vegas" + (vegas > 0 || ii.isCash(toScroll.getItemId())));
            }
            c.getSession().write(InventoryPacket.getInventoryFull());
            c.getSession().write(CWvsContext.enableActions());
            c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
        ...
        if (GameConstants.isTablet(scroll.getItemId()) && toScroll.getDurability() < 0) { //not a durability item
            if (chr.isShowErr()) {
                chr.showInfo("砸卷錯誤", true, "isTablet " + GameConstants.isTablet(scroll.getItemId()) + " getDurability " + (toScroll.getDurability() < 0));
            }
            c.getSession().write(InventoryPacket.getInventoryFull());
            c.getSession().write(CWvsContext.enableActions());
            c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
        ...
        if (!GameConstants.isMacHeart(toScroll.getItemId()) && (GameConstants.isTablet(scroll.getItemId()) || GameConstants.isGeneralScroll(scroll.getItemId()))) {//普通捲軸
            switch (scroll.getItemId() % 1000 / 100) {
                case 0: //單手1h
                    if (GameConstants.isTwoHanded(toScroll.getItemId()) || !GameConstants.isWeapon(toScroll.getItemId())) {
                        ...
                        c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
                        if (chr.isShowErr()) {
                            chr.showInfo("砸卷錯誤", true, "是雙手武器" + GameConstants.isTwoHanded(toScroll.getItemId()) + " 不是武器" + !GameConstants.isWeapon(toScroll.getItemId()));
                        }
                        return false;
                    }
                    break;
                case 1: //雙手2h
                    if (!GameConstants.isTwoHanded(toScroll.getItemId()) || !GameConstants.isWeapon(toScroll.getItemId())) {
                        ...
                        c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
                        if (chr.isShowErr()) {
                            chr.showInfo("砸卷錯誤", true, "不是雙手武器" + !GameConstants.isTwoHanded(toScroll.getItemId()) + " 不是武器" + !GameConstants.isWeapon(toScroll.getItemId()));
                        }
                        return false;
                    }
                    break;
                case 2: //防具armor
                    if (GameConstants.isAccessory(toScroll.getItemId()) || GameConstants.isWeapon(toScroll.getItemId())) {
                        ...
                        c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
                        if (chr.isShowErr()) {
                            chr.showInfo("砸卷錯誤", true, "是飾品" + GameConstants.isAccessory(toScroll.getItemId()) + " 是武器" + GameConstants.isWeapon(toScroll.getItemId()));
                        }
                        return false;
                    }
                    break;
                case 3: //飾品accessory
                    if (!GameConstants.isAccessory(toScroll.getItemId()) || GameConstants.isWeapon(toScroll.getItemId())) {
                        ...
                        c.getSession().write(CField.enchantResult(0, toScroll.getItemId()));
                        if (chr.isShowErr()) {
                            chr.showInfo("砸卷錯誤", true, "不是飾品" + !GameConstants.isAccessory(toScroll.getItemId()) + " 是武器" + GameConstants.isWeapon(toScroll.getItemId()));
                        }
                        return false;
                    }
                    break;
            }
        } else if (!GameConstants.isMacHeart(toScroll.getItemId()) && (GameConstants.isTMSSpecialScroll(scroll.getItemId()))) {
        ...
        } else if (!GameConstants.isAccessoryScroll(scroll.getItemId()) && //配飾
                !GameConstants.isChaosScroll(scroll.getItemId()) && //混沌捲軸
                !GameConstants.isCleanSlate(scroll.getItemId()) && //白醫
                !GameConstants.isEquipScroll(scroll.getItemId()) && //強化
                !GameConstants.isPotentialScroll(scroll.getItemId()) && //潛能
                !GameConstants.isBonusPotentialScroll(scroll.getItemId()) && //附加潛能
                !GameConstants.isSpecialScroll(scroll.getItemId()) &&//特殊捲軸
                !GameConstants.isResetScroll(scroll.getItemId()) && //還原捲軸
        ...
        if (scrolled == null) {
            ...
            if ((scroll.getItemId() / 100 == 20497 && scrolled.getState() == 1) || scrolled.getLevel() > oldLevel || scrolled.getEnhance() > oldEnhance || scrolled.getState() != oldState || scrolled.getBonusState() != oldBonusState || scrolled.getFlag() > oldFlag) {
                scrollSuccess = Equip.ScrollResult.SUCCESS;
            } else if ((GameConstants.isCleanSlate(scroll.getItemId()) && scrolled.getUpgradeSlots() > oldSlots)) {
                scrollSuccess = Equip.ScrollResult.SUCCESS;
            } else if ((GameConstants.isResetScroll(scroll.getItemId()) && scrolled != toScroll)) {
                scrolled = toScroll;
                scrollSuccess = Equip.ScrollResult.SUCCESS;
            } else {
                scrollSuccess = Equip.ScrollResult.FAIL;
    ...
    public static void UseCarvedSeal(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (Randomizer.nextInt(100) <= success) {
            ...
            while (!rewarded) {
                StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                if (pot != null && pot.reqLevel / 10 <= reqLevel && GameConstants.optionTypeFits(pot.optionType, eq.getItemId()) && GameConstants.potentialIDFits(pot.opID, new_state, 1)) { //optionType
                    if (GameConstants.isAllowedPotentialStat(eq, pot.opID, false)) {
    ...
    public static void UseAdditionalItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (Randomizer.nextInt(100) <= success) {
            ...
            final List<List<StructItemOption>> pots = new LinkedList<>(ii.getAllPotentialInfo().values());
            final int reqLevel = ii.getReqLevel(eq.getItemId()) / 10;
            int new_state = eq.getBonusState();
            if (new_state > 20 || new_state < 17) { // incase overflow
                new_state = 17;
            }
            //31001 = haste, 31002 = door, 31003 = se, 31004 = hb, 41005 = combat orders, 41006 = advanced blessing, 41007 = speed infusion
            ...
            while (!rewarded) {
                StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
                if (pot != null && pot.reqLevel / 10 <= reqLevel && GameConstants.optionTypeFits(pot.optionType, eq.getItemId()) && GameConstants.potentialIDFits(pot.opID, new_state, 1)) { //optionType
                    if (GameConstants.isAllowedPotentialStat(eq, pot.opID, true)) {
    ...
    public static void UseFlashCube(LittleEndianAccessor slea, MapleClient c) {
        ...
        c.getPlayer().updateTick(tick);
        final byte type = slea.readByte();
        switch (type) {
            case 1: {//放置楓方塊
                if (tick == 200315876) {
                    if (slea.readShort() == 1) {
                        if (c.getPlayer().getOneInfo(GameConstants.閃耀方塊, "p") == null) {
                            c.getSession().write(CWvsContext.enableActions());
                            return;
                        }
                        final Equip eq = (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(Short.valueOf(c.getPlayer().getOneInfo(GameConstants.閃耀方塊, "p")));
                        if (eq.getItemId() != Integer.valueOf(c.getPlayer().getOneInfo(GameConstants.閃耀方塊, "i"))) {
                            c.getSession().write(CWvsContext.enableActions());
                            return;
                        }
                        String[] s = c.getPlayer().getOneInfo(GameConstants.閃耀方塊, "o").split(",");
                        eq.setPotential1(Integer.valueOf(s[0]));
                        eq.setPotential2(Integer.valueOf(s[1]));
                        if (s.length == 3) {
                            eq.setPotential3(Integer.valueOf(s[2]));
                        }
                        c.getPlayer().forceReAddItem(eq, eq.getPosition() > 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED);
                    }
                    c.getPlayer().clearInfoQuest(GameConstants.閃耀方塊);
                    c.getSession().write(CField.getFlashCubeRespons(1, 1));
                } else if (tick == 1477907226) {
                    final short dst = slea.readShort();
                ...
                } else if (tick == 1098345356) {
                    int line = slea.readInt();
                    if (c.getPlayer().getOneInfo(GameConstants.閃炫方塊, "c") == null) {
                        c.getSession().write(CField.getShimmerCubeRespons(1, 0));
                        c.getSession().write(CWvsContext.enableActions());
                        return;
                    }
                    if (Integer.valueOf(c.getPlayer().getOneInfo(GameConstants.閃炫方塊, "c")) == line) {
                        final Equip eq = (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(Short.valueOf(c.getPlayer().getOneInfo(GameConstants.閃炫方塊, "p")));
                        if (eq.getItemId() != Integer.valueOf(c.getPlayer().getOneInfo(GameConstants.閃炫方塊, "i"))) {
                            c.getSession().write(CField.getShimmerCubeRespons(1, 0));
                            c.getSession().write(CWvsContext.enableActions());
                            return;
                        }
                        ...
                        for (int i = 0; i < pots.length; i++) {
                            String[] ss = c.getPlayer().getOneInfo(GameConstants.閃炫方塊, "o").split(",");
                            for (String s : ss) {
                        ...
                        for (int i = 0; i < pots.length; i++) {
                            if (i == 0) {
                                eq.setPotential1(pots[i]);
                            } else if (i == 1) {
                                eq.setPotential2(pots[i]);
                            } else if (i == 2) {
                                eq.setPotential3(pots[i]);
                            }
                        }
                        c.getPlayer().forceReAddItem(eq, eq.getPosition() > 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED);
                    }
                    c.getSession().write(CField.getShimmerCubeRespons(1, 1));
                    c.getPlayer().clearInfoQuest(GameConstants.閃炫方塊);
                }
            }
            case 3: {//洗方塊
                if (slea.available() == 8) {//楓方塊
                    ...
                    oldPots.add(eq.getPotential1());
                    oldPots.add(eq.getPotential2());
                    if (eq.getPotential3() > 0) {
                        oldPots.add(eq.getPotential3());
                    }
                    ...
                    if (c.getPlayer().useCube(toUse.getItemId(), eq)) {
                        ...
                        switch (toUse.getItemId()) {
                            ...
                            case 5062020://閃炫方塊
                                ...
                                newPots.add(eq.getPotential1());
                                newPots.add(eq.getPotential2());
                                if (eq.getPotential3() > 0) {
                                    newPots.add(eq.getPotential3());
                                }
                                eq.renewPotential(0);
                                InventoryHandler.magnifyEquip(c, null, eq, false);
                                newPots.add(eq.getPotential1());
                                ...
                                infoQuest = GameConstants.閃炫方塊;
                                c.getSession().write(CField.getShimmerCubeRespons(7, 0, eq.getPotential3() > 0 ? 3 : 2, newPots));
                                break;
                            default:
                                c.getPlayer().dropMessage(1, "此方塊未修復，請聯繫管理員。");
                                c.getSession().write(CWvsContext.enableActions());
                        ...
                        c.getPlayer().updateOneInfo(infoQuest, "c", "" + (1 + (eq.getPotential2() > 0 ? 1 : 0) + (eq.getPotential3() > 0 ? 1 : 0)));
                        c.getPlayer().updateOneInfo(infoQuest, "i", String.valueOf(eq.getItemId()));
                        c.getPlayer().updateOneInfo(infoQuest, "o", pots);
                        c.getPlayer().updateOneInfo(infoQuest, "p", String.valueOf(eq.getPosition()));
    ...
    public static boolean mapleCubeAction(MapleClient c, Equip eq, boolean use, int toLock, boolean free) {
        ...
        int potentialState = eq.getState();
        if (potentialState >= 17) {
            potentialState -= 16;
        }

        if (eq.getLevel() == 0 && eq.getUpgradeSlots() == 0 && !GameConstants.isShield(eq.getItemId()) && !GameConstants.isSpecialShield(eq.getItemId()) && !GameConstants.isEnergy(eq.getItemId()) && !GameConstants.isSpecialPotentialItem(eq.getItemId()) || MapleItemInformationProvider.getInstance().isCash(eq.getItemId()) || GameConstants.isSpecialCantPotentialItem(eq.getItemId())) {
            c.getPlayer().dropMessage(1, "在這道具無法使用。");
            c.getSession().write(CField.showMapleCubeCost(3, 0));
            return false;
        }

        if (GameConstants.isSpecialPotentialItem(eq.getItemId()) && potentialState == 0) {
        ...
        if (use) {
            long price = GameConstants.getMapleCubeCost(mapleCubePotTimes.get(state), potentialState);
            ...
            switch (toLock) {
                case 1:
                    lockPot = eq.getPotential1();
                    break;
                case 2:
                    lockPot = eq.getPotential2();
                    break;
                case 3:
                    lockPot = eq.getPotential3();
                    break;
                default:
                    lockPot = 0;
            }
            ...
            potentialState = eq.getState();
            if (potentialState >= 17) {
                potentialState -= 16;
            }
            state = potStates[potentialState];
        }
        c.getSession().write(CField.showMapleCubeCost(2, GameConstants.getMapleCubeCost(mapleCubePotTimes.get(state), potentialState)));
        return true;
    }
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !chr.hasBlockedInventory() && !chr.inPVP()) {
            switch (toUse.getItemId()) {
                ...
                case 5680021:
                    ...
                    break;
                case 2431965: //基本傷害肌膚 : 0
                case 2431966: //數位傷害肌膚 : 1
                case 2432084: //數位傷害肌膚 (no)
                case 2431967: //克里提亞斯傷害肌膚 : 2
                case 2432131: //組隊任務傷害肌膚 : 3
                case 2432153: //強烈傷害肌膚 : 4
                case 2432154: //甜蜜傳統韓?傷害肌膚 : 5
                case 2432207: //弓箭手村傷害字型 : 6
                case 2432354: //聖誕快樂傷害字型 : 7
                case 2432355: //雪花傷害字型 : 8
                case 2432465: //愛麗西亞的傷害字型 : 9
                case 2432479: //愛麗西亞的傷害字型 : 10
                case 2432526: //鍵盤戰盾傷害字型 : 11
                case 2432532: //輕飄飄春風傷害字型 : 12
                {
                    if (!MapleJob.is神之子(chr.getJob())) {
                        ...
                        int skinnum = GameConstants.getDamageSkinNumberByItem(sitemid);
        ...
        switch (itemId) {
            ...
            case 5062501: { //[MS特價] 大師附加奇幻方塊
                ...
                if (eq != null) {
                    ...
                    if ((line != 0 && GameConstants.canLockCube(itemId)) || line == 0) {
...
//                World.Broadcast.broadcastSmega(CWvsContext.broadcastMsg(3, c.getChannel(), c.getPlayer().getName() + " : " + message, ear));
            ...
            case 5155000: { // 卡勒塔的珍珠(更變精靈耳朵)
                ...
                if ((elf == 0 && !isMercedes) || (elf == 1 && isMercedes)) {
                    c.getSession().write(CField.showWeirdEffect("Effect/BasicEff.img/JobChangedElf", 5155000));
                    c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.showWeirdEffect(c.getPlayer().getId(), "Effect/BasicEff.img/JobChangedElf", 5155000), false);
                } else {
                    c.getSession().write(CField.showWeirdEffect("Effect/BasicEff.img/JobChanged", 5155000));
                    c.getPlayer().getMap().broadcastMessage(c.getPlayer(), CField.showWeirdEffect(c.getPlayer().getId(), "Effect/BasicEff.img/JobChanged", 5155000), false);
                }
            ...
            case 5156000: { // 偉大的變身之藥
                if (c.getPlayer().getMarriageId() > 0) {
                    ...
                    c.getSession().write(CField.EffectPacket.showOwnCraftingEffect("Effect/BasicEff.img/TransGender", (byte) 1, 0, 0));
...
//                    System.err.println("Return 12");
    ...
    public static boolean checkPotentialLock(MapleCharacter chr, Equip eq, int line, int potential) {
        ...
        if (line == 1 && eq.getPotential1() != potential
                || line == 2 && eq.getPotential2() != potential
                || line == 3 && eq.getPotential3() != potential) {
