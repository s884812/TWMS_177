...
public final class MonsterBook
    ...
    public MonsterBook(Map<Integer, Integer> cards, MapleCharacter chr) {
        ...
        if ((stat != null) && (stat.getCustomData() != null)) {
            ...
            if ((!this.sets.containsKey(this.currentSet)) || (!((Boolean) ((Pair) this.sets.get(this.currentSet)).right))) {
                this.currentSet = -1;
            }
        }
        applyBook(chr, true);
...
//        }
    ...
    public final boolean monsterCaught(MapleClient c, int cardid, String cardname) {
        if ((!this.cards.containsKey(cardid)) || ((this.cards.get(cardid)) < 2)) {
            this.changed = true;
//            c.getPlayer().dropMessage(-6, new StringBuilder().append(cardname).append("已成功登記至怪物卡冊.").toString());
...
//                }
//                c.getSession().write(CField.EffectPacket.showForeignEffect(61));
//                if (rr > 1) {
