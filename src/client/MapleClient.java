...
//                System.out.println("密碼哈希值 - " + passhash);
                    ...
                    if (getLoginState() > MapleClient.LOGIN_NOTLOGGEDIN) { // already loggedin
                        if (loginok != 0) {
                            ...
                            for (final MapleClient c : World.Client.getClients()) {
                                if (c.getAccID() == accId) {
                                    if (c.getSessionIPAddress().equals(getSessionIPAddress())) {
                                        c.disconnect(true, getChannel() == -10);
    ...
    public final void disconnect(final boolean unLock) {
    ...
    public int getCharacterSlots() {
        ...
        return charslots + (30 - DEFAULT_CHARSLOT);
    }
