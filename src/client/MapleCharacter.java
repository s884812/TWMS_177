...
public class MapleCharacter extends AnimatedMapleMapObject implements Serializable, MapleCharacterLook {
    ...
    public static MapleCharacter getDefault(final MapleClient client, final JobType type) {
        ...
        ret.map = null;
        ret.exp = 0;
        ret.gmLevel = (byte) (ServerConstants.isBetaForAdmins ? PlayerGMRank.ADMIN.getLevel() : client.getGmLevel());
        if (ServerConstants.isBetaForAdmins) {
    ...
    public static void saveNewCharToDB(final MapleCharacter chr, final JobType type, short db, int keymapType) {
        ...
        try {
            ...
            ps = con.prepareStatement("INSERT INTO characters (level, str, dex, luk, `int`, hp, mp, maxhp, maxmp, sp, hsp, ap, skincolor, gender, job, hair, face, faceMarking, ears, tail, map, meso, party, buddyCapacity, pets, subcategory, elf, friendshippoints, chatcolour, gm, accountid, name, world)"
...
//            }
    ...
    public void registerEffect(MapleStatEffect effect, long starttime, ScheduledFuture<?> schedule, Map<MapleBuffStat, Integer> statups, boolean silent, final int localDuration, final int cid) {
        ...
        if (isShowInfo()) {
            showMessage(25, new StringBuilder().append("註冊BUFF效果 - 當前BUFF總數： ").append(this.effects.size()).append(" 技能： ").append(effect.getSourceId()).toString());
        }
    }
    ...
    public void cancelEffect(final MapleStatEffect effect, final boolean overwrite, final long startTime, Map<MapleBuffStat, Integer> statups) {
        ...
        if (isShowInfo()) {
            dropMessage(5, new StringBuilder().append("取消BUFF效果 - 當前BUFF總數：").append(this.effects.size()).append(" 技能：").append(effect.getSourceId()).toString());
        }
...
//                        this.client.getSession().write(CWvsContext.BuffPacket.giveEnergyChargeTest(energyLevel, echeff.getDuration() / 1000));
                        this.client.getSession().write(CWvsContext.BuffPacket.giveBuff(0, 0, stat, echeff));
...
//            client.getSession().write(EffectPacket.updateDeathCount(99)); //for fun
    ...
    public void changeJob(short newJob) {
        try {
            ...
            map.broadcastMessage(this, EffectPacket.showForeignEffect(getId(), 0xD), false); // 變更職業
...
//        System.out.println(start + " | " + end);
        ...
        if (linkId > 0) {
            ...
            if (getSkills().get(SkillFactory.getSkill(linkId)).skillLevel < nowLinkLevel) {
...
//            updateDonorMedal(getInventory(MapleInventoryType.EQUIPPED).findById(1142229));
    ...
    public int getTotalSkillLevel(final Skill skill) {
        ...
        if (GameConstants.isItemSkill(this, skill.getId())) {
            return 1;
        }
        int skillLevel;
    ...
    public void levelUp() {
        ...
        stats.setInfo(maxhp, maxmp, maxhp, maxmp);
        client.getSession().write(CWvsContext.updatePlayerStats(statup, this));
        map.broadcastMessage(this, EffectPacket.showForeignEffect(getId(), 0), false); // 升級
        characterCard.recalcLocalStats(this);
        stats.recalcLocalStats(this);
        silentPartyUpdate();
        guildUpdate();
...
//            client.getSession().write(PetPacket.petStatUpdate(this));
    ...
    public void modifyCSPoints(int type, int quantity, boolean show) {
        ...
        if (show && quantity != 0) {
            ...
            client.getSession().write(EffectPacket.showForeignEffect(22));
        }
    }
...
//        client.getSession().write(PetPacket.petStatUpdate(this));
    ...
    public void resetStats(final int str, final int dex, final int int_, final int luk) {
        ...
        client.getSession().write(EffectPacket.showWZEffect(0x1A, "Effect/OnUserEff.img/RecordClear_BT/clear"));//0x1A 173ok
    }
    ...
    public void updateAndroid(boolean itemOnly) {
        if (this.map != null) {
            this.map.broadcastMessage(CField.updateAndroidLook(itemOnly, this, android));
        }
    }
    ...
    public Map<Byte, Integer> getSecondEquips(boolean fusionAnvil) {
        ...
        for (final Item item : inventory[MapleInventoryType.EQUIPPED.ordinal()].newList()) {
            ...
            if (item instanceof Equip) {
                ...
                if (MapleJob.is天使破壞者(getJob()) && GameConstants.isOverall(itemId)) {
...
//        client.getSession().write(EffectPacket.updateDeathCount(deathCount));
    ...
    public boolean useCube(int itemId, Equip eq, int toLock) {
        if (getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
            ...
            if (!GameConstants.canUseCube(eq, itemId)) {
                dropMessage(5, "在這道具無法使用。");
                return false;
            }
            if ((!isBonus && eq.getState() >= 17 && eq.getState() <= 20) || (isBonus && (eq.getBonusState() >= 17 && eq.getBonusState() <= 20))) {
                if (isShowInfo() && ((isBonus && eq.getBonusState() < 20) || (!isBonus && eq.getState() < 20))) {
                    dropMessage(-6, "伺服器管理員跳框成功率100%");
                }
                if (!isBonus) {
                    eq.renewPotential(isAdmin() ? 99 : stateRate, GameConstants.getCubeType(itemId), toLock);
                } else {
                    eq.renewBonusPotential(isAdmin() ? 99 : stateRate);
                }
                forceReAddItem_NoUpdate(eq, MapleInventoryType.EQUIP);

                if (GameConstants.getCubeFragment(itemId) > 0) {
                    MapleInventoryManipulator.addById(getClient(), GameConstants.getCubeFragment(itemId), (short) 1, new StringBuilder().append("使用方塊獲得").append(FileoutputUtil.CurrentReadable_Date()).toString());
                }
                return true;
            } else {
    ...
    public boolean changeFace(int color) {
        ...
        if (!GameConstants.faceList.containsKey(f)) {
            return false;
        }
        face = f;
