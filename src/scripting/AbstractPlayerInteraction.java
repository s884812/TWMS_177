    ...
    public final void gainItem(final int id, final short quantity, final boolean randomStats, final long period, boolean hours, final int slots, final String owner, final MapleClient cg, final boolean show) {
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(id) && !GameConstants.isBullet(id)) {
                ...
                item.setGMLog("從腳本 " + this.id + " (" + id2 + ") 中獲得, 時間 " + FileoutputUtil.CurrentReadable_Time());
                ...
                MapleInventoryManipulator.addById(cg, id, quantity, owner == null ? "" : owner, null, period, hours ? MapleInventoryManipulator.HOUR : MapleInventoryManipulator.DAY, "從腳本 " + this.id + " (" + id2 + ") 中獲得, 時間 " + FileoutputUtil.CurrentReadable_Time());
            }
        } else {
...
//        System.err.println(curtime);
    ...
    public void showWZEffect(final int value, final String data) {
        c.getSession().write(EffectPacket.showWZEffect(value, data));
    }
    ...
    public void gainPet(int id, String name, int level, int closeness, int fullness, long period, short flags) {
        ...
        try {
            MapleInventoryManipulator.addById(c, id, (short) 1, "", MaplePet.createPet(id, name, level, closeness, fullness, MapleInventoryIdentifier.getInstance(), id == 5000054 ? (int) period : 0, flags, 0), 45, MapleInventoryManipulator.DAY, "從腳本 " + id + " (" + id2 + ")" + " 中獲得, 時間:" + FileoutputUtil.CurrentReadable_Date());
